...
Running the code chunk below creates a simple linear regression model where `shares` is the response variable and the predictor variables are `weekday`, `num_imgs`, `num_keywords`, `n_tokens_title`, and `title_subjectivity`. The `summary()` function is used to examine the values for the residuals and coefficients, as well as the performance criteria values such as multiple R-squared.

```{r}
set.seed(100)
secondLinearModel <- train(shares ~ weekday + num_imgs + num_keywords + n_tokens_title + title_subjectivity, 
                        data = newsTrain,
                        method = "lm",
                        preProcess = c("center", "scale"),
                        trControl = trainControl(method = "cv"))
...
#confusionMatrix(randomForestPredict, newsTest$shares)
```
...
```{r}
boostTreeFit <- train(shares ~ weekday + num_imgs + num_keywords + n_tokens_title + title_subjectivity, data = newsTrain,
                        method = "gbm",
                        preProcess = c("center", "scale"),
...
Now that the boosted tree model has been trained (`boostTreeFit`), running the code chunk below will check how well the model does on the test set `newsTest` using the `confusionMatrix()` function. The `data` given to the confusion matrix was the `shares` variable from the `newsTest` data frame as it was this variable the boosted tree model was trained on. 
...
#confusionMatrix(boostingPredict, newsTest$shares)
```
